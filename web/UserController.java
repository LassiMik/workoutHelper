...
import com.example.workoutHelper.domain.SignupForm;
    ...
    public String save(@Validated @ModelAttribute("signupform") SignupForm signupForm, BindingResult bindingResult) {
    	if (!bindingResult.hasErrors()) { // validation errors
    		if (signupForm.getPassword().equals(signupForm.getPasswordCheck())) { // check password match		
		    	...
		    	newUserClass.setPasswordHash(hashPwd);
		    	newUserClass.setUsername(signupForm.getUsername());
		    	newUserClass.setRole("USER");
		    	if (repository.findByUsername(signupForm.getUsername()) == null) { // Check if user exists
		    		repository.save(newUserClass);
		    	}
		    	else {
